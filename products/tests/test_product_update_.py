...
class ProductUpdateTest(TestCase):
    ...
    # Test product update with a negative price
    def test_product_update_with_negative_price(self):
        old_price = self.product.price
        ...
        # Negative price should be rejected if the form/model validation
        # does not allow negative values
        self.assertEqual(response.status_code, 200)

        self.product.refresh_from_db()

        # Existing price should remain unchanged
        self.assertEqual(self.product.price, old_price)
